...
/// For TheMinerzCoin, set to 10 hours, denominated in seconds.
...
{
    return (GetTime<std::chrono::seconds>() - m_best_block_time.load()).count() / m_chainparams.GetConsensus().nTargetSpacing;
}

bool PeerManagerImpl::CanDirectFetch()
{
    return m_chainman.ActiveChain().Tip()->Time() > NodeClock::now() - m_chainparams.GetConsensus().TargetSpacing() * 20;
}
...
bool PeerManagerImpl::ProcessNetBlock(const std::shared_ptr<const CBlock> pblock, bool force_processing, bool min_pow_checked, bool* new_block, CNode& pfrom)
{
    ...
    // TheMinerzCoin ToDo: revert after nodes upgrade to current version
    // /*
...
{
    // TheMinerzCoin: Do not ask for NODE_WITNESS for now
    /*
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    /*
    // TheMinerzCoin: Do not ignore pre-segwit peers for now
    ...
    */
    ...
    /*
    // TheMinerzCoin: Do not send witness flag for now
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    ...
                    peer->m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                        (
                         ...
                         Ticks<std::chrono::seconds>(NodeClock::now() - m_chainman.m_best_header->Time()) / consensusParams.nTargetSpacing
                        );
                    nSyncStarted++;
                }
            }
