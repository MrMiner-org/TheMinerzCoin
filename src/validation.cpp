    ...
    */
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        ...
        if (minFee > 0 && package_fee < minFee) {
            return state.Invalid(TxValidationResult::TX_RECONSIDERABLE, "min fee not met", strprintf("%d < %d", package_fee, minFee));
        }
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    */
    ...
    // TheMinerzCoin also requires DER encoding of pubkeys and low S in sigs
...
{
    // TheMinerzCoin ToDo: enable the check for PoW headers
    /*
 ...
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, CChain& chain, bool fProofOfStake) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    AssertLockHeld(::cs_main);
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked, bool fOldClient)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // TheMinerzCoin ToDo: remove fOldClient after the nodes got updated
        ...
        if (!ContextualCheckBlockHeader(block, state, m_blockman, *this, pindexPrev, chainstate.m_chain, fSetAsProofOfStake)) {
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // TheMinerzCoin: also check for the signature encoding
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    if (!ContextualCheckBlockHeader(block, state, chainstate.m_blockman, chainstate.m_chainman, pindexPrev, chainstate.m_chain, block.IsProofOfStake()))
