...
UniValue getstakinginfo(const UniValue& params, bool fHelp)
{
    ...
    int64_t nTargetSpacing = consensusParams.GetDynamicTargetSpacing(currentHeight);
...
UniValue checkkernel(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        ...
        if (vNodes.empty())
            throw JSONRPCError(-9, "TheMinerzCoin is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(-10, "TheMinerzCoin is downloading blocks...");
