...
gen-theminerzcoin-conf.sh
===================

Generates a bitcoin.conf file in `share/examples/` by parsing the output from `blackmored --help`. This script is run during the
release process to include a theminerzcoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/build contrib/devtools/gen-theminerzcoin-conf.sh
```
