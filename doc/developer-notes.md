...
- [Development guidelines](#development-guidelines)
    - [General TheMinerzCoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
TheMinerzCoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
TheMinerzCoin is a multi-threaded application, and deadlocks or other
...
TheMinerzCoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as TheMinerzCoin, where everyone uses
...
pay attention to for reviewers of TheMinerzCoin code.

General TheMinerzCoin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: TheMinerzCoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of TheMinerzCoin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against TheMinerzCoin so that they can be integrated
...
file descriptors, it will cause problems with TheMinerzCoin's `select()` loop, because
...
compatibility between nodes. This happened in TheMinerzCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to TheMinerzCoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of TheMinerzCoin, the wallet was always
