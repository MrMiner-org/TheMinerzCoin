...
cd "${BASE_BUILD_DIR}/theminerzcoin-$HOST"
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  set -eo pipefail
  cd "${BASE_BUILD_DIR}/theminerzcoin-$HOST/src/"
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/theminerzcoin-$HOST/contrib/devtools/iwyu/bitcoin.core.imp" \
